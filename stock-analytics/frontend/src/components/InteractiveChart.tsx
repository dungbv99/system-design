...
import {
  calcMA, calcEMA, calcBB, calcRSI, calcMACD, calcStoch, calcVWAP,
  calcSuperTrend, calc52WHL, calcAroon, calcADX, calcCCI, calcATR,
  calcWilliamsR, calcOBV, calcBBW, calcIchimoku, calcParabolicSAR,
  calcDonchian, calcPivotPoints, calcFibLevels, calcMFI, calcROC,
  calcCMF, calcWyckoffClimax,
...
export function InteractiveChart({ quotes, indicators }: Props) {
  const [tf, setTf] = useState<Timeframe>('D')

  const activeQuotes = useMemo(() => {
    if (tf === 'W') return aggregateWeekly(quotes)
    if (tf === 'M') return aggregateMonthly(quotes)
    return quotes
  }, [quotes, tf])
  ...
  const paneLabel = (id: string, label: string, ref: React.RefObject<HTMLDivElement>) =>
    indicators.has(id) && (
      <>
        <div className="text-xs text-[#8b949e]/60 px-1 pt-1">{label}</div>
        <div ref={ref} className="w-full" />
      </>
    )

  const TF_OPTIONS: { id: Timeframe; label: string; count: string }[] = [
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="flex items-center justify-between mb-1.5 flex-wrap gap-2">
        <div className="flex gap-1">
          {TF_OPTIONS.map(opt => (
            ...
            >
              {opt.label}
              <span className={`ml-1 text-[9px] ${tf === opt.id ? 'text-[#0d1117]/70' : 'text-[#8b949e]/60'}`}>
                {opt.count}
              </span>
            </button>
          ))}
      ...
      <div className="text-xs text-[#8b949e]/40 text-right pt-0.5">scroll to zoom · drag to pan</div>
    </div>
  )
}
