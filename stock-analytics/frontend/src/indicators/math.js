...
export function calcBB(closes, n = 20, mult = 2) {
...
export function calcStoch(quotes, k = 14, d = 3) {
...
export function calcVWAP(quotes, period = 20) {
...
export function calcSuperTrend(quotes, period = 10, mult = 3) {
...
export function calcADX(quotes, period = 14) {
    const n = quotes.length;
...
export function calcCCI(quotes, period = 20) {
    return quotes.map((_, i) => {
        if (i < period - 1)
            return null;
        const sl = quotes.slice(i - period + 1, i + 1);
...
export function calcATR(quotes, period = 14) {
...
export function calcWilliamsR(quotes, period = 14) {
    return quotes.map((_, i) => {
        if (i < period - 1)
            return null;
        const sl = quotes.slice(i - period + 1, i + 1);
...
export function calcOBV(quotes) {
...
export function calcBBW(closes, n = 20, mult = 2) {
    return calcBB(closes, n, mult).map(v => v == null ? null : (v.upper - v.lower) / v.mid * 100);
}
export function calcIchimoku(quotes) {
...
export function calcParabolicSAR(quotes, afStep = 0.02, afMax = 0.2) {
...
export function calcDonchian(quotes, period = 20) {
    return quotes.map((_, i) => {
        if (i < period - 1)
...
export function calcPivotPoints(quotes) {
    return quotes.map((_, i) => {
        if (i === 0)
...
export function calcFibLevels(quotes) {
...
export function calcMFI(quotes, period = 14) {
...
export function calcROC(closes, period = 14) {
    return closes.map((c, i) => i < period ? null : ((c - closes[i - period]) / closes[i - period]) * 100);
}
export function calcCMF(quotes, period = 20) {
    return quotes.map((_, i) => {
        if (i < period - 1)
            return null;
        let mfv = 0, vol = 0;
...
export function calcWyckoffClimax(quotes, period = 20, mult = 2.5) {
